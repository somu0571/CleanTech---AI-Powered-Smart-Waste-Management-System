...
def render_auth():
    ...
    with col:
        ...
        with tab_login:
            ...
            role = st.selectbox(
                ...
                format_func=lambda x:
                    "👤 Citizen" if x == "citizen"
                    else "🧑‍💼 Admin" if x == "admin"
                    else "♻️ Recycler"
            )
        ...
        with tab_reg:
            ...
            r_recycler_id = r_center = None

            if r_role == "admin":
                st.info("🔐 Admin verification required")
                r_admin_id = st.text_input("Official Admin ID", key="r_aid")
                r_org = st.text_input("Organisation Name", key="r_org")

            elif r_role == "recycler":
                st.info("♻️ Recycler verification required")
                r_recycler_id = st.text_input("Recycler ID", key="r_rid")
                r_center = st.text_input("Recycling Center Name", key="r_center")
...
def render_sidebar():
    with st.sidebar:
        ...
        if st.session_state.role == "admin":
            role_icon = "🧑‍💼"
        elif st.session_state.role == "recycler":
            role_icon = "♻️"
        else:
            role_icon = "👤"
        ...
        if st.session_state.role == "admin":
            ...
            pages = {
                "🏠 Recycler Dashboard": "recycler_home",
                "♻️ Pickup Requests": "pickup",
...
# RECYCLER PAGES
# ══════════════════════════════════════════════════════════════════════════════
def page_recycler_dashboard():
    st.markdown("""
    <div class="page-header">
      <h1>👷 Recycler Dashboard</h1>
      <p>Manage pickups and track performance</p>
    </div>
    """, unsafe_allow_html=True)

    # Fetch data
    r1 = api("get", "/recycler/available")
    r2 = api("get", "/recycler/my-jobs")

    available = r1.json() if r1 and r1.status_code == 200 else []
    my_jobs = r2.json() if r2 and r2.status_code == 200 else []

    pending = len(available)
    total_jobs = len(my_jobs)
    completed = len([j for j in my_jobs if j["status"] == "completed"])

    col1, col2, col3, col4 = st.columns(4)

    kpi(col1, "📥", pending, "Available Requests")
    kpi(col2, "🚚", total_jobs, "My Jobs")
    kpi(col3, "✅", completed, "Completed")
    kpi(col4, "🌱", st.session_state.eco_points, "Eco Points")
...
def page_pickup_requests():
    st.markdown("""
    <div class="page-header">
      <h1>📥 Pickup Requests</h1>
      <p>Accept waste collection jobs</p>
    </div>
    """, unsafe_allow_html=True)

    r = api("get", "/recycler/available")
    ...
    st.markdown(f"### Total Requests: {len(data)}")
    ...
    for c in data:
        ...
        with col2:
            if st.button("Accept", key=f"accept_{c['id']}"):
                res = api("post", f"/recycler/accept/{c['id']}")
...
def page_my_jobs():
    ...
    <div class="page-header">
      <h1>🚚 My Pickups</h1>
      <p>Track your assigned jobs</p>
    </div>
    """, unsafe_allow_html=True)

    r = api("get", "/recycler/my-jobs")
...
def page_admin_home():
    ...
    stats = {}
    if r:
        st.write("Status:", r.status_code)   # DEBUG (remove later)
        if r.status_code == 200:
            stats = r.json()
        else:
            st.write("Error:", r.text)

    # ───────────────── KPI CARDS ─────────────────
    c1, c2, c3, c4 = st.columns(4)

    kpi(c1, "👥", stats.get("total_citizens", "–"), "Total Citizens")
    ...
    with col2:
        ...
        if rh and rh.status_code == 200:
            ...
            zones = [z["zone"] for z in hm]
            vals = [z["waste_kg"] for z in hm]

            fig = go.Figure(go.Bar(x=zones, y=vals))
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col3:
        waste_input = st.number_input("Waste (kg)", min_value=0, value=500)
        recycled = st.slider("Recycling %", 0, 100, 60)
    ...
    with col4:
        ...
        kpi(c_b, "🌳", round(co2_saved / 21.7, 1), "Tree Equivalent")
        kpi(c_c, "⛽", round(co2_saved / 2.31, 1), "Fuel Offset")
...
def page_heatmap():
    ...
    # ✅ Cache API data (prevents reload flicker)
    ...
    # ✅ Create map ONLY once (prevents flicker)
    if "heatmap_map" not in st.session_state:
        ...
        # Optional markers (clickable zones)
        for z in hm:
            folium.CircleMarker(
                ...
                fill_color="red",
                fill_opacity=0.9,
                popup=f"{z['zone']} - {z['waste_kg']:.1f} kg",
                tooltip=z["zone"]
            ).add_to(m)

        st.session_state.heatmap_map = m
    ...
    with col1:
        st_folium(
            st.session_state.heatmap_map,
            width="100%",
            height=450,
            returned_objects=[]  # ✅ prevents rerun flicker
        )
...
def page_recycle():
    st.title("♻️ Recycle Guide")

    st.markdown("### Learn how to recycle properly 🌱")

    st.write("🟢 Plastic → Clean & separate")
    st.write("📄 Paper → Keep dry")
    st.write("🥫 Metal → Rinse & crush")
    st.write("🍾 Glass → Separate by color")

    st.success("Small steps → Big environmental impact 🌍")

def page_recycler_home():
    st.title("🧑‍🔧 Recycler Dashboard")
    st.info("Manage pickups and track collected waste")

def page_pickup():
    st.title("♻️ Pickup Requests")

    st.write("List of pending waste pickups")

    # Example UI
    st.warning("Pickup Request: Plastic Waste - Sector 12")
    if st.button("Accept Pickup"):
        st.success("Pickup accepted 🚛")

def page_collected():
    st.title("📦 Collected Waste")

    st.write("Track collected materials")

    st.success("Plastic: 120kg")
    st.success("Metal: 80kg")
...
def main():
    ...
    "recycle":      page_recycle,
    "report":       page_report,
    "ecopoints":    page_ecopoints,

    # Recycler ✅ CORRECT
    "recycler_home": page_recycler_dashboard,
    "pickup":        page_pickup_requests,
    "collected":     page_my_jobs,
}
